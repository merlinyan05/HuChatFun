"""
HuChatFun V2.4 QLoRA 训练脚本
在 RTX 5080 16GB 上微调 Qwen3-8B
变更：LoRA r=128（V2.3 是 r=64），其余参数不变
用法: python training/v2.4/train.py
"""
...
OUTPUT_DIR = str(ROOT / "models" / "huchat-lora-v6")
LOG_DIR = str(ROOT / "logs" / "run6")

# LoRA 参数 —— V2.4: r=128（V2.3 是 r=64）
LORA_R = 128
LORA_ALPHA = 256            # alpha = 2*r，保持 alpha/r 比例不变
LORA_DROPOUT = 0.05
...
MAX_SEQ_LEN = 1024
BATCH_SIZE = 1
GRAD_ACCUM = 16            # 有效 batch = 16（OOM 则改 32，时间翻倍但省显存）
LR = 1.5e-4
EPOCHS = 3
WARMUP_RATIO = 0.03
NEFTUNE_ALPHA = 5
...
def main():
    ...
    print(f"  LoRA rank: {LORA_R}（V2.3 是 64）")
